...
public interface GameRepository extends JpaRepository<PingpongGame, Long> {
	...
	List<PingpongGame> findByTeamAndDateBetween(@Param("team") PingpongTeam team, @Param("startDate") Date startDate, @Param("endDate") Date endDate);

	@Query("FROM PingpongGame WHERE weekNo=:week AND date BETWEEN :startDate AND :endDate")
	List<PingpongGame> findBySeasonWeek(@Param("week") Integer week, @Param("startDate") Date startDate, @Param("endDate") Date endDate);

	@Query("FROM PingpongGame WHERE weekNo=:week AND (homeTeam=:team OR outTeam=:team) AND date BETWEEN :startDate AND :endDate")
	PingpongGame findByTeamInSeasonWeek(@Param("team") PingpongTeam team, @Param("startDate") Date startDate, @Param("endDate") Date endDate, @Param("week") int week);
	...
	List<PingpongGame> findByCalendarWeek(@Param("week") Integer week, @Param("startDate") Date startDate, @Param("endDate") Date endDate);

	@Query("FROM PingpongGame WHERE WEEK(date, 3)=:week AND (homeTeam=:team OR outTeam=:team) AND date BETWEEN :startDate AND :endDate")
	PingpongGame findByTeamInCalendarWeek(@Param("team") PingpongTeam team, @Param("startDate") Date startDate, @Param("endDate") Date endDate, @Param("week") int week);
