...
public class PlayerServiceImpl implements PlayerService {
	...
	public List<PingpongPlayer> getPlayers(boolean recreation) {
		return repository.findByActiveAndRecreationOrderByRankingAscLastNameAsc(true, recreation);
	}
	...
	public void generateAndEmailNewPassword(PingpongPlayer player) throws InvalidEmailException {
		...
		repository.save(player);
		mailService.sendEmail(player, "Nieuw wachtwoord", 
				"Beste "+player.getFullName()+",\n\n"+
	...
	public PingpongPlayer updatePassword(PingpongPlayer player, String oldPassword, String newPassword, String repeatNewPassword) throws InvalidPasswordUpdateException {
	...
	public PingpongPlayer updateProfile(PingpongPlayer player) throws InvalidEmailException {
		...
		if(!oldPlayer.getAddress().equals(player.getAddress())) {
			fieldUpdates.add(new FieldUpdate("Adres", oldPlayer.getAddress().getFullAddress(", "), player.getAddress().getFullAddress(", ")));
		}
		if(!oldPlayer.getPhoneNumbers().equals(player.getPhoneNumbers())) {
			fieldUpdates.add(new FieldUpdate("Telefoonnummers", StringUtils.join(oldPlayer.getPhoneNumbers(), ", "), StringUtils.join(player.getPhoneNumbers(), ", ")));
		}
